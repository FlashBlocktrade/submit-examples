...
func ping(ep Endpoint) pingRes {
    ...
    if err != nil { return pingRes{EP: ep, OK: false, MS: ms} }
...
func selectBestEndpoint(preferred string) Endpoint {
    ...
    for _, ep := range endpoints { go func(e Endpoint){ ch <- ping(e) }(ep) }
    results := make([]pingRes, 0, len(endpoints))
    for range endpoints { results = append(results, <-ch) }
    healthy := make([]pingRes, 0)
    for _, r := range results { if r.OK { healthy = append(healthy, r) } }
    list := results
    if len(healthy) > 0 { list = healthy }
...
func submitBatch(baseURL, auth string, txs []string) (submitResp, error) {
    ...
    for {
        ...
        resp, err := httpClient.Do(req)
        if err != nil { if attempts < 2 { attempts++; continue }; return submitResp{}, err }
        defer resp.Body.Close()
        var m map[string]any
        _ = json.NewDecoder(resp.Body).Decode(&m)
        if resp.StatusCode == 429 || resp.StatusCode >= 500 { if attempts < 2 { attempts++; continue } }
...
func extractSignatures(raw map[string]any) (bool, int64, string, []string) {
    ...
    if d, ok := top["data"].(map[string]any); ok {
        if arr, ok := d["signatures"].([]any); ok {
            for _, x := range arr { if s, ok := x.(string); ok { sigs = append(sigs, s) } }
        }
    }
    if len(sigs) == 0 { if arr, ok := top["signatures"].([]any); ok { for _, x := range arr { if s, ok := x.(string); ok { sigs = append(sigs, s) } } } }
